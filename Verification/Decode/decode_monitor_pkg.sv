package decode_monitor_pkg;
    ...
    class decode_monitor #(parameter int DATA_WIDTH = 32 , ADDR_WIDTH = 32) extends uvm_monitor;

        //Register the class into the factory
        `uvm_component_param_utils(decode_monitor #(DATA_WIDTH,ADDR_WIDTH))
        ...
        uvm_analysis_port #(decode_item #(DATA_WIDTH,ADDR_WIDTH)) mon_port;
        decode_item #(DATA_WIDTH,  ADDR_WIDTH) mon_item;
        virtual decode_interface #(.DATA_WIDTH(DATA_WIDTH),.ADDR_WIDTH(ADDR_WIDTH)) vif;

        virtual function void build_phase (uvm_phase phase);
            super.build_phase(phase);
            mon_port = new("mon_port",this);
            mon_item = decode_item #(DATA_WIDTH,ADDR_WIDTH)::type_id::create("mon_item");
